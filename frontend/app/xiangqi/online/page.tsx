...
export default function XiangqiQueuePage() {
  ...
  useEffect(() => {
    if (hydrated && !accessToken) {
      router.replace("/login?next=/xiangqi/online");
    }
  ...
  useEffect(() => {
    ...
    const offMsg = socket.onMessage((msg) => {
      if (msg.type === "match_found" && msg.variant === "xiangqi") {
        searchingRef.current = false;
        router.push(`/xiangqi/online/${msg.game_id}`);
      }
      if (msg.type === "error") {
        searchingRef.current = false;
        setSearching(false);
  ...
  const startSearch = () => {
    ...
    const socket = getGameSocket();
    socket.connect();
    socket.send({ type: "join_queue", time_control: timeControl, variant: "xiangqi" });
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-12">
      ...
      <p className="mt-2 text-sm text-muted">
        Ghép cặp theo Elo cờ tướng riêng (±100, nới dần khi chờ lâu). Elo cờ
        tướng của bạn:{" "}
