...
export default function CaroQueuePage() {
  ...
  useEffect(() => {
    if (hydrated && !accessToken) {
      router.replace("/login?next=/caro/online");
    }
  ...
  useEffect(() => {
    ...
    const offMsg = socket.onMessage((msg) => {
      if (msg.type === "match_found" && msg.variant === "caro") {
        searchingRef.current = false;
        router.push(`/caro/online/${msg.game_id}`);
      }
      if (msg.type === "error") {
        searchingRef.current = false;
        setSearching(false);
    ...
    const offStatus = socket.onStatus((status) => {
      ...
      if (status === "open" && searchingRef.current) {
        socket.send({ type: "join_queue", time_control: timeControl, variant: "caro" });
      }
    });
    return () => {
      offMsg();
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-12">
      ...
      <p className="mt-2 text-sm text-muted">
        Ghép cặp theo Elo caro riêng (±100, nới dần khi chờ lâu). Elo caro của
        bạn:{" "}
