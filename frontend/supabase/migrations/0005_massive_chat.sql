...
ALTER TABLE "game_rooms" ADD CONSTRAINT "ck_game_rooms_type" CHECK ("game_rooms"."game_type" in ('chess', 'xiangqi', 'caro', 'jungle', 'oanquan', 'uno', 'tienlen', 'ngua', 'xidach', 'baicao'));--> statement-breakpoint
ALTER TABLE "game_rooms" ADD CONSTRAINT "ck_game_rooms_shape" CHECK ((
        ...
        or
        ("game_rooms"."game_type" in ('xidach', 'baicao') and "game_rooms"."max_players" between 2 and 4 and "game_rooms"."time_control" is null)
        or
        ("game_rooms"."game_type" not in ('uno', 'tienlen', 'ngua', 'xidach', 'baicao') and "game_rooms"."max_players" = 2 and "game_rooms"."time_control" is not null)
      ));